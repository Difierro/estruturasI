...
int main(void){
    ...
    //funcao para escrever dados no saida_q3.txt
...
void getQtdAlunos(FILE * entrada, int * qtdAlunos){
    int c;
    while( (c = fgetc(entrada)) != EOF){ //enquanto receber caractere diferente de EOF
        if(c == '\n'){
            (*qtdAlunos)++;
        }
    }
...
/* \fcanf == pega informações especificas, a gente coloca l + foarmatação (%20["\n"]\t%f\t%f\t%f) */
